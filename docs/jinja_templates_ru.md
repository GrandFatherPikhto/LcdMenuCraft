...
Публичный хелпер `menu_draw_pad_marker(ctx)` ([`menu_draw.h`](../output/include/menu_draw.h))
дописывает маркер состояния на **правый край** строки: `>` в режиме навигации,
`*` при редактировании. Он сам меряет `strlen(ctx->value_buf)`, так что вызывающему
коду не нужно знать ширину строки. Позиция маркера вычисляется, а не зашита числом:

```c
#define MENU_LINE_LEN (LCD_STRING_LEN / LCD_NUM_STRINGS)   // 32 / 2 = 16
```

поэтому она остаётся корректной при смене дисплея или `LCD_STRING_LEN`. Функция
теперь публичная (раньше была `static`) специально для того, чтобы её можно
было вызывать напрямую из своего `draw_value_cb`, а не копировать логику
паддинга/маркера в каждый проект вручную — именно такое копирование и привело
к тому, что копия молча разошлась с генератором.
